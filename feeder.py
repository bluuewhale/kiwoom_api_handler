...
class Feeder:
    ...
    def getOPTKWFID(self, codeList):
        ...
        for start, end in zip(
            range(0, len(codeList), 100), range(100, len(codeList) + 100, 100)
        ):
            # input params
    ...
    def getCodesFromKRX(self, market="all"):
        ...
        if market == "all":
            kospiCodeList = list(
                pd.read_html(kospiURL, header=0)[0]["종목코드"].map("{:06d}".format)
            )
            kosdaqCodeList = list(
                pd.read_html(kosdaqURL, header=0)[0]["종목코드"].map("{:06d}".format)
            )
