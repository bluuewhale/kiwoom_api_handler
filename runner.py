...
class OvernightReversalRunner:
    ...
    async def __asyncSendBuyOrderAtOpen(self):
        ...
        buyOrderDictListAtOpen = self.orderCreator.getBuyOrderDictListAtOpen(
            **paramDict
        )
    ...
    async def __asyncSendStairLimitBuyOrder(self):
        ...
        self.__sendStairLimitBuyOrder(
            buyCodeList=buyCodeList, avgBuyVolume=avgBuyVolume
        )
    ...
    async def __asyncSendStairLimitSellOrder01(self):
        ...
        self.__sendStairLimitSellOrder(sellCodeList=sellCodeList, sellWeight=1.0)
    ...
    def __sendOpenMarketBuyOrder(
        self, buyCodeList, indMinBuyVolume, indMaxBuyVolume, totalMaxBuyVolume
    ):
        """ 개장 직후 시장가 + 지정가 매수 주문 """
    ...
    def __sendMarketSellOrder(self, sellCodeList, sellWeight=1.0):
        ...
        marketSellOrderDictList = self.orderCreator.getMarketSellOrderDictList(
            accountNum=self.accountNum, sellCodeList=sellCodeList, sellWeight=sellWeight
        )
    ...
    def __sendCancelOrder(self, cancelCodeList):
        ...
        while True:
            ...
            cancelOrderDictList = self.orderCreator.getCancelOrderDictList(
                accountNum=self.accountNum, cancelCodeList=cancelCodeList
            )
    ...
    def checkServerStatus(self):
        try:
            ...
            self.logger.error(
                "{} Server Connection : Fail => Trying to reconnect to Server".format(
                    datetime.now()
                )
            )
            self.connect()
